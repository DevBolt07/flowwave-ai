...
export const AuthorityDashboard = ({ onBack }: AuthorityDashboardProps) => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'control' | 'config'>('overview');
  ...
  const handleManualOverride = (action: 'green' | 'red' | 'reset') => {
    toast({
      title: "Manual Override",
      description: `${action.charAt(0).toUpperCase() + action.slice(1)} signal command sent to intersection`,
  ...
  const handleDetectionToggle = () => {
    ...
    toast({
      ...
      description: detectionRunning ? "AI vehicle detection has been paused" : "AI vehicle detection is now active",
                      ...
                      <div key={intersection.id} className="relative">
                        ...
                        <div className="absolute top-2 right-2">
                          <Badge variant="outline" className="text-xs">
                            {intersection.detection.model} • {intersection.detection.confidence * 100}%
                          </Badge>
                        </div>
                      </div>
                    ))}
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="text-xs space-y-1">
                        <div className="flex justify-between">
                          <span>Model:</span>
                          <span className="font-mono">{selectedIntersectionData.detection.model}</span>
                        </div>
                        <div className="flex justify-between">
                          <span>Confidence:</span>
                          <span className="font-mono">{(selectedIntersectionData.detection.confidence * 100).toFixed(1)}%</span>
                        </div>
                        <div className="flex justify-between">
                          <span>FPS:</span>
                          <span className="font-mono">{selectedIntersectionData.detection.fps}</span>
                        </div>
                      </div>
                    </div>
                  </CardContent>
